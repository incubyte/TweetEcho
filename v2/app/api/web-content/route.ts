...
export async function GET(request: NextRequest) {
  try {
    ...
    if (url) {
      const content = await webContentService.getWebContentByUrl(
        userId || user.id,
        url
      );
    ...
    const contents = await webContentService.getUserWebContent(
      userId || user.id
    );
    ...
    return NextResponse.json(
      { error: "Failed to get web content" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to delete web content" },
      { status: 500 }
    );
  }
}
